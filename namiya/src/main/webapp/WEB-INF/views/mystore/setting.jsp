...
function setSelectedMenu() {
	...
	setMenu('rabit',50,50,32,32);
	setMenu('bt_32',50,50,32,32);
	setMenu('bt_32',50,50,32,32);
	setMenu('bt_32',50,50,32,32);
}
...
function setMenu(filename,px,py,width,height) {
	...
	imgObject.py = py;
	imgObject.filename = filename;
	var name = 'resources/img/furniture/'+imgObject.filename+imgObject.currentframe+'.png';
	var img = new Image();
	img.src = name;
	img.onload = imageLoaded;
	imgObject.source = img;
...
function changeOrder(sel) {
	var cnt = -1;
	cnt = isItemColli(sel,imgList[sel].px,imgList[sel].py,imgList[sel].width,imgList[sel].height);
...
function isItemColli(sel,px,py,width,height) {
	var x1 = px;
...
function furnitureItem(searchItem,searchWord,gap) {
	currentFurniturePage = Number(currentFurniturePage) + Number(gap);
	
	$.ajax({
		method : 'GET'
		,url : 'reqFurniture'
		,data : 'currentPage='+currentFurniturePage+'&searchItem='+searchItem+'&searchWord='+searchWord
		,success : imageOutput}
	);
	
}

function backgroundItem(searchItem,searchWord,gap) {
currentBackgroundPage = Number(currentBackgroundPage) + Number(gap);  

	$.ajax({
		method : 'GET'
		,url : 'reqFurniture'
		,data : 'currentPage='+currentBackgroundPage+'&searchItem='+searchItem+'&searchWord='+searchWord
		,success : imageOutput}
	);
}
